...
| Submit task or **chat file** shows **413 Request Entity Too Large** (nginx HTML) | On VPS nginx `server` block add `client_max_body_size 6m;` then `sudo nginx -t && sudo systemctl reload nginx` (see `deploy/nginx-upload-limit.conf.example`) |
